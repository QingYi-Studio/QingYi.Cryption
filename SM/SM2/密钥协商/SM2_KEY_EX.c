...
int SM2_Init()
{
    ...
    coordinates if (!epoint_set(para_Gx, para_Gy, 0, G)) // initialise point G
    {
...
int SM2_KeyEx_Init_II(big ra, big dA, epoint *RA, epoint *RB, epoint *PB, unsigned char ZA[], unsigned char ZB[], unsigned char SB[], unsigned char K[], int klen, unsigned char SA[])
{
    ...
    SM3_KDF(Z, SM2_NUMWORD * 2 + SM3_len / 4, klen / 8, K);
    //---------------A9:(optional) S1 =
    Hash(0x02 || Uy || Hash(Ux || ZA || ZB || x1 || y1 || x2 || y2))-- -- -- -- -- -SM3_init(&md);
    SM3_process(&md, Z, SM2_NUMWORD);
    SM3_process(&md, ZA, SM3_len / 8);
    SM3_process(&md, ZB, SM3_len / 8);
    SM3_process(&md, x1y1, SM2_NUMWORD * 2);
...
int SM2_KeyEX_SelfTest()
{
    // standard data
    unsigned char
        std_priKeyA[SM2_NUMWORD] = {0x81, 0xEB, 0x26, 0xE9, 0x41, 0xBB, 0x5A, 0xF1, 0x6D, 0xF1, 0x16, 0x49, 0x5F, 0 x90, 0x69, 0x52, 0x72, 0xAE, 0x2C, 0xD6, 0x3D, 0x6C, 0x4A, 0xE1, 0x67, 0x84, 0x18, 0xBE, 0x48, 0x23, 0x00, 0x2 9};
    unsigned char
    ...
    unsigned char
        std_priKeyB[SM2_NUMWORD] = {0x78, 0x51, 0x29, 0x91, 0x7D, 0x45, 0xA9, 0xEA, 0x54, 0x37, 0xA5, 0x93, 0x56, 0 xB8, 0x23, 0x38,
    ...
    unsigned char
        std_RA[SM2_NUMWORD * 2] = {0x64, 0xCE, 0xD1, 0xBD, 0xBC, 0x99, 0xD5, 0x90, 0x04, 0x9B, 0x43, 0x4D, 0x0F, 0xD7, 0x34, 0x28, 0xCF, 0x60, 0x8A, 0x5D, 0xB8, 0xFE, 0x5C, 0xE0, 0x7F, 0x15, 0x02, 0x69, 0x40, 0xBA, 0xE4, 0x0E,

                                   0x37, 0x66, 0x29, 0xC7, 0xAB, 0x21, 0xE7, 0xDB, 0x26, 0x09, 0x22, 0x49, 0x9D, 0xDB, 0x11, 0x8F, 0x07, 0xCE, 0x 8E, 0xAA, 0xE3, 0xE7, 0x72, 0x0A, 0xFE, 0xF6, 0xA5, 0xCC, 0x06, 0x20, 0x70, 0xC0};
    unsigned char
